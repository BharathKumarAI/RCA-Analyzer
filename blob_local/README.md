...
Existing explicit canonical-store overrides remain supported. No existing data is moved by initialization. Pin old canonical paths before changing defaults; see [migration notes](../docs/blob-storage.md#deployment-and-existing-installations). Git and container builds exclude real project content. Compose mounts platform read-only and projects read-write.
