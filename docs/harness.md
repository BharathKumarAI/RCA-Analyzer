...
- [Evidence-backed visual answers](#evidence-backed-visual-answers)
...
Connector evidence has its own optional model-profile alias, `evidence`, and stage prompt. New baseline profiles map it to the separately configurable evidence model stage; disabling the logs model does not disable this branch when the evidence mapping remains enabled. Older saved profiles without that alias retain their configured logs mapping. Older six-key prompt documents remain valid: the resolver uses their saved logs instruction, after project overrides, as the evidence instruction. This compatibility behavior does not enable a connector or add actions. Sources: [profiles](../app/models/profiles.py), [baseline models](../blob_local/platform/config/model_profiles.yaml), [prompt validation](../app/configuration/platform.py), [layer resolution](../app/configuration/layers.py).

Per-capability approved Harness bundles can select a profile alias or an existing enabled model stage ID in `x-rca.stage_model`. Studio's agent inspector edits that value in the source bundle, subject to validation and independent approval. The run freezes resolved base, specialist and bundle model configurations, together with the effective prompts, before execution. Native assembly reads those saved values. Additional source notes enter synthesis as unverified context; only the recorded evidence ledger supplies citable observations. Sources: [bundle validation](../app/configuration/harness_bundles.py), [Studio inspector](../frontend/src/features/harness-studio/inspector/InspectorPanel.tsx), [snapshot creation](../app/runtime/runner.py), [native assembly](../app/agents/root.py). [Stage regression checks](../tests/unit/test_evidence_stages.py) cover independent evidence stages, saved prompt compatibility, named model stages and frozen configuration.

## Evidence-backed visual answers

The native `InvestigationResult` may include up to six typed visual objects, with a combined serialized size of at most 64 KiB. They are saved with the terminal result and returned through the same run API. The synthesizer remains a tool-free native ADK agent; visual output does not introduce another model or execution engine. Sources: [result contract](../app/runtime/run_contract.py), [visual schemas](../app/runtime/visuals.py), [synthesis agent](../app/agents/rca_synthesizer.py).

| Kind | Data contract | Workspace interaction |
| --- | --- | --- |
| `chart` | Bar or line chart, explicit axis labels and unit, up to 48 labeled numerical or unknown values | Keyboard-selectable points, source inspection and a data-table view; unknown measurements remain gaps |
| `table` | Up to eight typed columns and 60 cited rows; numerical columns require units | Sortable headers, explicit unknown cells and row source links |
| `code` | Up to 8,000 characters with a supported language label and provenance, including `mermaid` | Copyable text; Mermaid uses the same isolated diagram renderer as fenced answer blocks |
| `graph`, `service_map`, `blast_radius` | Up to 30 cited nodes and 60 cited edges referencing existing node IDs; blast radius requires explicit focus nodes | Keyboard-selectable nodes, relationship inspection, observed-only relationship filter and node table |

Every point, row, code excerpt, node and edge declares `observed` or `inferred` and cites saved evidence. Graph status can remain `unknown`; an unknown status never renders as healthy. The schema rejects duplicate IDs, dangling graph references, non-finite numbers, incompatible table cell types and executable rendering fields. At terminal validation, every citation must belong to the current run. This validates provenance and shape; it does not independently prove the model's interpretation of a source or its causal claims. No visual is generated as an application fallback when evidence is absent.

Chat uses React and SVG to render the saved structures and reuses its evidence inspector for source links. Mermaid code visuals and fenced Markdown diagrams share a bounded renderer with strict Mermaid settings. Configuration directives, custom styles, node metadata and external assets are blocked before rendering; node clicks and callbacks stay disabled. The displayed SVG uses an empty iframe sandbox and a content security policy that blocks external resources. Raw HTML and executable snippets remain inert; invalid or unsupported diagrams retain a readable source view. Prefer the typed graph kinds for incident topology because they retain citations for each node and edge. Narrative diagrams explain recorded findings and must not invent relationships or measured values. Existing summary/table presentation metadata remains available for results without typed visuals. Sources: [visual renderer](../frontend/src/components/chat/ChatAnswerVisuals.tsx), [safe Markdown rendering](../frontend/src/components/AnswerMarkdown.tsx), [diagram boundary](../frontend/src/components/MermaidBlock.tsx), [generation guidance](../blob_local/platform/config/prompts.yaml), [schema checks](../tests/unit/test_visual_contract.py), [native workflow persistence checks](../tests/integration/test_visual_answers.py), [interaction checks](../tests/frontend_answer_visuals.mjs). These tests use isolated verification fixtures and do not establish live connector reachability or model quality.
...
Interactive investigations do not automatically resume after process failure. Scheduled knowledge capture, closure monitoring and controlled improvement use a separate [leased SQL job queue](../app/optimization/improvement.py); they do not authorize arbitrary uploaded-code execution or source-system writes. Saved investigation traces make execution inspectable, not resumable by themselves. The triage proposal endpoint delegates saved follow-up questions through the governed runner, documented in [the workspace boundary](project.md#triage-workspace-implementation-boundary). It cannot execute arbitrary connector queries or external writes.
