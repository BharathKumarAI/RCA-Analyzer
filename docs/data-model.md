...
A row in triage `investigation_evidence` is not automatically captured ADK-run evidence. The intake endpoint copies validated redacted evidence bundles from a completed live run and retains their run/evidence IDs and hashes. Follow-up execution delegates to the governed runner and records its result before promotion. Legacy rows lacking that provenance must not be treated as verified source observations. Empty projects are never seeded by reads. Sources: [triage migration](../migrations/history/028_triage_board.sql), [triage API](../app/api/routes/triage.py).
...
This source cleanup does not delete previously persisted rows in an operator database. Existing demo rows cannot safely be distinguished from later user-edited records by a ticket ID alone; any database cleanup needs a reviewed scoped record list and backups. Fabricated response cards, methodology analyses and synthetic proposal execution have been removed; see the recorded-run workspace contract. Sources: [triage handlers](../app/api/routes/triage.py), [empty-board regression](../tests/integration/test_triage_board_api.py), [isolated fixture](../tests/triage_support.py), [triage boundary](project.md#triage-workspace-implementation-boundary).
