...
The answer renderer treats Markdown as untrusted and avoids raw HTML/scripts/remote image loading. Original files are separately owned downloads, not guaranteed-redacted copies. Keep raw originals out of ordinary evidence previews. Sources: [file parsing](../app/inputs/files.py), [upload API](../app/api/routes/files.py), [safe rendering](../frontend/src/components/AnswerMarkdown.tsx), [artifact downloads](../app/api/routes/chats.py), [result validation](../app/runtime/runner.py).
...
The [OKF integration design](knowledge.md#okf-security-and-attestation-boundary) keeps imported trust claims separate from local authorization and review. Proposed import/export must retain scope checks, bounded data parsing, protected originals and non-executable content. No OKF application endpoints are implemented by this design.
