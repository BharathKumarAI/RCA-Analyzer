...
import { downloadKnowledgeDoc, fetchKnowledge, fetchPrincipal, reviewKnowledgeDoc } from '../services/api';
...
export const Knowledge: React.FC = () => {
  ...
  const load = async () => {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Knowledge could not load. Refresh to try again.'); }
  ...
  const saved = (item: KnowledgeItem) => {
    ...
    setSelectedId(item.id); setEditor(null); setQuery(''); setStatusFilter('all');
    setNotice('Document saved as a draft. Review the content, then submit it for approval.');
  };
  ...
  const filtered = items.filter(item => (statusFilter === 'all' || item.status === statusFilter) && [item.title, item.category, item.content, ...item.tags].join(' ').toLowerCase().includes(query.trim().toLowerCase()));
  ...
  return <div className="view-container knowledge-page">
    ...
    {loading && !items.length ? <div className="knowledge-loading" role="status">Loading project knowledge…</div> : <div className="knowledge-workspace">
      <aside className="knowledge-library" aria-label="Knowledge library">
        <div className="knowledge-library-controls"><h2>Documents <span>{items.length}</span></h2><label className="knowledge-search"><Search size={16} aria-hidden="true" /><span className="knowledge-visually-hidden">Search documents</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search documents" /></label><label>Show<select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="all">All documents</option>{['draft', 'pending', 'approved', 'rejected', 'revoked', 'stored'].map(status => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></label></div>
        {filtered.length ? <ul>{filtered.map(item => <li key={item.id}><button type="button" className={selectedId === item.id && editor !== 'new' ? 'is-selected' : ''} aria-pressed={selectedId === item.id && editor !== 'new'} onClick={() => select(item.id)}><strong>{item.title}</strong><span>{item.category}</span><span className="knowledge-status">{statusLabel(item.status)}</span></button></li>)}</ul> : <div className="knowledge-empty"><BookOpen size={24} aria-hidden="true" /><h3>{items.length ? 'No matching documents' : 'Add your first document'}</h3><p>{items.length ? 'Try another search or show all documents.' : canManage ? 'Start with a runbook that helps your team investigate incidents.' : 'A project owner can add documents for your team.'}</p>{items.length > 0 && <button type="button" className="btn btn-secondary" onClick={() => { setQuery(''); setStatusFilter('all'); }}>Clear filters</button>}</div>}
      </aside>
      <section className="knowledge-detail" aria-label="Document details">
        {editor === 'new' || (editor === 'edit' && selected) ? <><header><h2>{editor === 'new' ? 'Add project knowledge' : 'Edit document'}</h2><p>Save a draft first. Review and approval happen before this content is used.</p></header><KnowledgeDocumentForm key={editor === 'edit' ? selected!.id : 'new'} item={editor === 'edit' ? selected! : undefined} onSaved={saved} onCancel={() => setEditor(null)} /></> : selected ? <>
          ...
          <section><h3>{selected.upload ? 'Extracted content' : 'Document content'}</h3><pre className="knowledge-content">{selected.content}</pre></section>
