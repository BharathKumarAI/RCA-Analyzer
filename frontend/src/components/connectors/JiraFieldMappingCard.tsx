...
export const JiraFieldMappingCard = ({ mappings, onChange, readOnly = false }: JiraFieldMappingCardProps) => {
  ...
  return <section className="connector-field-mapping" aria-label="Jira custom field mapping">
    ...
    {visible.map(row => <div className="connector-mapping-row" key={row.id}>
      <label>Jira field ID<input className="rca_assist-input mono" value={row.sourceField} pattern="customfield_[0-9]{1,12}" placeholder="customfield_…" disabled={readOnly} onChange={event => onChange(mappings.map(item => item.id === row.id ? {...item, sourceField: event.target.value} : item))} /></label>
      <label>Evidence label<input className="rca_assist-input" value={row.targetField} maxLength={128} disabled={readOnly} onChange={event => onChange(mappings.map(item => item.id === row.id ? {...item, targetField: event.target.value} : item))} /></label>
    ...
    {!visible.length && <p className="rca_assist-field-hint">{search ? 'No fields match your search.' : 'No custom fields mapped. Add IDs verified in your Jira instance.'}</p>}
