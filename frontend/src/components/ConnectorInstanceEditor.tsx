...
export const ConnectorInstanceEditor: React.FC<ConnectorInstanceEditorProps> = ({
  ...
  const renderGovernanceBadge = (fieldName: string) => {
    ...
    if (isPlatformAdmin && !readOnly && !projectContext) {
      return (
        <div className="rca_assist-field-gov-control" title="Field Access & Control: Manage visibility and editability for project users">
  ...
  const [customFieldMappings, setCustomFieldMappings] = useState<CustomFieldRow[]>(() => {
    ...
    if (mapping && typeof mapping === 'object' && !Array.isArray(mapping)) return Object.entries(mapping).map(([id, label]) => ({ sourceField: id, rca_assistField: String(label), fieldType: 'string', required: false, defaultValue: '' }));
  ...
  const currentCandidate = useMemo(() => {
    ...
    return {
      ...
      custom_field_mapping: Object.fromEntries(customFieldMappings.map(field => [field.sourceField, field.rca_assistField])),
  ...
  const handleSave = async () => {
    ...
    if (customFieldMappings.some(field => !/^customfield_[0-9]{1,12}$/.test(field.sourceField) || !field.rca_assistField.trim()) || new Set(customFieldMappings.map(field => field.sourceField)).size !== customFieldMappings.length || new Set(customFieldMappings.map(field => field.rca_assistField.trim().toLowerCase())).size !== customFieldMappings.length) {
  ...
  return (
    <div className="rca_assist-connector-container" role="region" aria-label="Connector Configuration Studio">
      {/* ====================================================================
      ...
      <header className="rca_assist-header">
        <div className="rca_assist-header-main">
          <div className="rca_assist-breadcrumbs">
          ...
          <div className="rca_assist-title-row">
            <div className="rca_assist-icon-badge">
              <Database size={20} />
            </div>
            <div>
              <h2 className="rca_assist-title">
                {systemName || template?.name || 'Connector Configuration'}
                <span className={`rca_assist-status-pill ${savedInstance?.status === 'enabled' ? 'active' : 'draft'}`}>
                  <span className="dot" />
                  {savedInstance?.status || 'draft'}
                </span>
                {isPolicyBlocked && (
                  <span className="rca_assist-status-pill policy-blocked">
                    <ShieldAlert size={11} /> Unavailable for project setup
                  </span>
                )}
              </h2>
              <p className="rca_assist-subtitle">
        ...
        <div className="rca_assist-header-controls">
          <div className="rca_assist-scope-pill" title="Server-authenticated project scope">
            <ShieldCheck size={12} />
            <span>Scope: <b>{projectId}</b></span>
          </div>

          <div className="rca_assist-env-badge" title="Selected runtime environment">
            <Globe size={12} />
            <span>Env: <b>{toolEnvironment || 'Shared'}</b></span>
          </div>

          <span className="rca_assist-scope-pill">{template?.protocol || 'Connector'} · v{template?.version || '1.0.0'}</span>
        </div>
      </header>
      ...
      {actionError && (
        <div className="rca_assist-alert-banner error" role="alert">
          <AlertCircle size={15} />
          <span>{actionError}</span>
        </div>
      )}
      {actionSuccess && (
        <div className="rca_assist-alert-banner success" role="status">
      ...
      {/* ====================================================================
          ...
          ==================================================================== */}
      <div className="rca_assist-content-grid">
        {/* ------------------------------------------------------------------
        ...
        <div className="rca_assist-grid-col">
          ...
          {(
            <section className="rca_assist-card" id="section-connection">
              <div className="rca_assist-card-header">
                <div className="rca_assist-card-title-wrap">
                  <div className="rca_assist-card-icon"><Tag size={16} /></div>
                  <div>
                    <h2 className="rca_assist-card-title">Basic Information</h2>
                    <p className="rca_assist-card-desc">Name, target, ownership, and environment settings for this project connection.</p>
                  </div>
                </div>
                <div className="rca_assist-brand-tag">
                  {template?.category || 'Core Service'}
                </div>
              </div>

              <div className="rca_assist-card-body">
                <div className="rca_assist-form-grid">
                  {/* System Name (Mandatory Project-Only) */}
                  <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('system_name') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label htmlFor="system-name-input" className="rca_assist-field-label">
                    ...
                    <input
                      ...
                      required
                    />
                    <span className="rca_assist-field-hint">Project-owned runtime identity. Persisted independently of template renames.</span>
                  </div>

                  {/* Connector Type (Template-Derived) */}
                  <div className="rca_assist-field-group">
                    <div className="rca_assist-field-header">
                      <label className="rca_assist-field-label">Connector Type</label>
                      <span className="rca_assist-gov-badge project-locked"><Lock size={10} /> Platform Managed</span>
                    </div>
                    <input
                      type="text"
                      className="rca_assist-input locked"
                  ...
                  {/* Endpoint URL */}
                  <div className="rca_assist-field-group full-width" hidden={!isPlatformAdmin && getFieldTier('endpoint') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label htmlFor="endpoint-url" className="rca_assist-field-label">
                    ...
                    <input
                      ...
                      required
                    />
                    <span className="rca_assist-field-hint">{endpointHint}</span>
                  </div>

                  {/* Environment Dependency Segmented Control */}
                  <div className="rca_assist-field-group full-width">
                    <div className="rca_assist-field-header">
                      <label className="rca_assist-field-label">
                        Environment Dependency <span className="req">*</span>
                      </label>
                      {renderGovernanceBadge('environment_dependency')}
                    </div>
                    <div className="rca_assist-toggle-group">
                      <button
                        type="button"
                        className={`rca_assist-toggle-btn ${environmentDependency === 'independent' ? 'active' : ''}`}
                      ...
                      <button
                        type="button"
                        className={`rca_assist-toggle-btn ${environmentDependency === 'dependent' ? 'active' : ''}`}
                    ...
                    <span className="rca_assist-field-hint">
                      {environmentDependency === 'dependent'
                  ...
                  {/* Tool Environment */}
                  <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('tool_environment') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label htmlFor="tool-env-input" className="rca_assist-field-label">
                  ...
                  {/* Owner */}
                  <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('owner') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label htmlFor="owner-input" className="rca_assist-field-label">Owner</label>
                  ...
                  {/* Description */}
                  <div className="rca_assist-field-group full-width" hidden={!isPlatformAdmin && getFieldTier('description') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label htmlFor="desc-input" className="rca_assist-field-label">Description</label>
                      {renderGovernanceBadge('description')}
                    </div>
                    <textarea
                      id="desc-input"
                      className="rca_assist-textarea"
                      rows={2}
                  ...
                  <div className="rca_assist-field-group full-width" hidden={!isPlatformAdmin && getFieldTier('tags') === 'platform_only'}>
                    <label className="rca_assist-field-label">Tags</label>
                    <div className="rca_assist-chip-wrap">
                      {tags.map(t => (
                        <span key={t} className="rca_assist-chip">
                          {t}
                          {!readOnly && !isFieldLocked('tags') && (
                            <button
                              type="button"
                              onClick={() => handleRemoveTag(t)}
                              className="rca_assist-chip-remove"
                      ...
                      {!readOnly && !isFieldLocked('tags') && (
                        <div className="rca_assist-chip-input-wrap">
                          <input
                            type="text"
                            value={newTagInput}
                            ...
                            placeholder="Add tag..."
                            className="rca_assist-chip-input"
                          />
                          <button
                            type="button"
                            onClick={handleAddTag}
                            className="rca_assist-chip-add-btn"
                          >
                            <Plus size={12} />
                          </button>
                        </div>
          ...
          {(
            <section className="rca_assist-card" id="section-authentication">
              <div className="rca_assist-card-header">
                <div className="rca_assist-card-title-wrap">
                  <div className="rca_assist-card-icon"><Key size={16} /></div>
                  <div>
                    <h2 className="rca_assist-card-title">Authentication</h2>
                    <p className="rca_assist-card-desc">Supported credential references and secure provider protocols.</p>
                  </div>
                </div>
                <div className="rca_assist-card-action">
                  {visibleTestResult?.overall_result === 'PASSED' ? (
                    <span className="rca_assist-badge-configured"><CheckCircle2 size={12} /> Verified</span>
                  ) : (
                    <span className="rca_assist-badge-unverified"><Clock size={12} /> Unverified</span>
                  )}
                </div>
              </div>
              ...
              <div className="rca_assist-card-body">
                ...
                <div className="rca_assist-form-grid" style={{ marginTop: '14px' }}>
                  ...
                  {selectedCredentialFields.length > 0 ? selectedCredentialFields.filter(field => isPlatformAdmin || (getFieldTier('credentials') !== 'platform_only' && getFieldTier(field) !== 'platform_only')).map(field => {
                    ...
                    return (
                      <div className="rca_assist-field-group" key={field}>
                        <div className="rca_assist-field-header">
                          <label htmlFor={fieldId} className="rca_assist-field-label">
                        ...
                        <input
                          id={fieldId}
                          type="text"
                          className={`rca_assist-input ${secretReference ? 'mono' : ''}`}
                    ...
                    <div className="rca_assist-empty-state full-width">
                ...
                <div className="rca_assist-test-connection-strip">
                  <button
                    type="button"
                  ...
                  <div className="rca_assist-test-result-indicator">
                    {visibleTestResult?.overall_result === 'PASSED' ? (
                      <span className="rca_assist-test-success">
                        <CheckCircle2 size={13} />
                        <span>Connection successful ({visibleTestResult.latency_ms}ms)</span>
                      </span>
                    ) : visibleTestResult?.overall_result === 'FAILED' ? (
                      <span className="rca_assist-test-failure">
                        <AlertTriangle size={13} />
                        <span>Verification failed</span>
                      </span>
                    ) : (
                      <span className="rca_assist-test-idle">
                        <Clock size={13} />
          ...
          {(
            <section className="rca_assist-card" id="section-environments" hidden={!isPlatformAdmin && getFieldTier('environment_connections') === 'platform_only'}>
              <div className="rca_assist-card-header">
                <div className="rca_assist-card-title-wrap">
                  <div className="rca_assist-card-icon"><Server size={16} /></div>
                  <div>
                    <h2 className="rca_assist-card-title">Environment connections</h2>
                    <p className="rca_assist-card-desc">
              ...
              <div className="rca_assist-card-body">
                ...
                {isConnModalOpen && editingConn && (
                  <div className="rca_assist-conn-form-card" role="region" aria-label="Connection Form">
                    <div className="rca_assist-conn-form-header">
                      <div className="rca_assist-conn-form-title">
                        <Server size={14} />
                    ...
                    <div className="rca_assist-form-grid">
                      <div className="rca_assist-field-group">
                        <label className="rca_assist-field-label" htmlFor="env-connection-name">Connection Name <span className="req">*</span></label>
                      ...
                      <div className="rca_assist-field-group">
                        <label className="rca_assist-field-label" htmlFor="env-connection-environment">Target Environment <span className="req">*</span></label>
                      ...
                      <div className="rca_assist-field-group full-width">
                        <label className="rca_assist-field-label">Routing Mode <span className="req">*</span></label>
                        <div className="rca_assist-toggle-group">
                          <button
                            type="button"
                            className={`rca_assist-toggle-btn ${editingConn.routing_mode === 'direct' ? 'active' : ''}`}
                          ...
                          <button
                            type="button"
                            className={`rca_assist-toggle-btn ${editingConn.routing_mode === 'mcp' ? 'active' : ''}`}
                          ...
                          <button
                            type="button"
                            className={`rca_assist-toggle-btn ${editingConn.routing_mode === 'hybrid' ? 'active' : ''}`}
                      ...
                      <div className="rca_assist-field-group full-width">
                        <label className="rca_assist-field-label" htmlFor="env-connection-auth">Authentication Profile <span className="req">*</span></label>
                      ...
                      <div className="rca_assist-field-group full-width">
                        <label className="rca_assist-field-label">Target Endpoint / URL <span className="req">*</span></label>
                        <input
                          type={endpointInputType}
                          className="rca_assist-input mono"
                          ...
                          placeholder={endpointPlaceholder}
                        />
                        <span className="rca_assist-field-hint">Environment-specific service URL. Secrets and tokens are strictly forbidden in URL endpoints.</span>
                      </div>
                      ...
                      {profileCredentialFields(
                        ...
                        return (
                          <div key={field} className="rca_assist-field-group">
                            <label className="rca_assist-field-label">
                              {credentialFieldLabel(field)}
                              {isSecret && <span className="rca_assist-gov-badge project-locked"><Lock size={10} /> Secret Ref</span>}
                            </label>
                            <input
                            ...
                            />
                            {isSecret && (
                              <span className="rca_assist-field-hint">Use an approved environment variable reference (env://NAME).</span>
                            )}
                          </div>
                        );
                      })}
                      ...
                      <div className="rca_assist-field-group full-width">
                        <label className="rca_assist-field-label" htmlFor="connection-resource-scope">Allowed resources</label>
                        <textarea id="connection-resource-scope" className="rca_assist-input" rows={3}
                          value={(editingConn.resource_scope || []).join('\n')}
                          onChange={event => setEditingConn({ ...editingConn, resource_scope: event.target.value.split('\n') })} />
                        <span className="rca_assist-field-hint">One project, index, namespace, topic or path per line. Environment assignments must use one of these resources.</span>
                      </div>
                      {(editingConn.routing_mode === 'mcp' || editingConn.routing_mode === 'hybrid') && <>
                        <div className="rca_assist-field-group">
                          <label className="rca_assist-field-label" htmlFor="connection-mcp-endpoint">MCP endpoint</label>
                          <input id="connection-mcp-endpoint" type="url" className="rca_assist-input" value={editingConn.mcp_configuration?.endpoint || ''}
                            onChange={event => setEditingConn({ ...editingConn, mcp_configuration: { ...editingConn.mcp_configuration, endpoint: event.target.value } })} />
                        </div>
                        <div className="rca_assist-field-group">
                          <label className="rca_assist-field-label" htmlFor="connection-mcp-token">MCP token reference</label>
                          <input id="connection-mcp-token" className="rca_assist-input" placeholder="env://MCP_TOKEN" value={editingConn.mcp_configuration?.token_secret_ref || ''}
                        ...
                        <div className="rca_assist-field-group full-width">
                          <label className="rca_assist-field-label" htmlFor="connection-mcp-tools">MCP operation mapping (JSON)</label>
                          <textarea ref={mcpMappingInput} id="connection-mcp-tools" className="rca_assist-input mono" rows={5} defaultValue={JSON.stringify(editingConn.mcp_configuration?.mcp_tools || {}, null, 2)}
                          ...
                          <span className="rca_assist-field-hint">Map each operation to its server tool name, scope argument, arguments and argument map.</span>
                        </div>
                        {editingConn.routing_mode === 'hybrid' && <div className="rca_assist-field-group full-width">
                          <label className="rca_assist-field-label" htmlFor="connection-mcp-route">Evidence route</label>
                          <select id="connection-mcp-route" className="rca_assist-select" value={editingConn.mcp_configuration?.operation_routes?.[template?.system_name === 'itsm' ? 'get_ticket' : template?.system_name === 'log_search' ? 'query_range' : 'read_evidence'] || ''}
                    ...
                    <div className="rca_assist-conn-form-footer">
                      <button
                        type="button"
                        className="btn btn-secondary"
                        onClick={() => {
                ...
                {connections.length === 0 ? (
                  <div className="rca_assist-empty-state" style={{ padding: '24px 16px', textAlign: 'center' }}>
                  ...
                  <div className="rca_assist-conn-table-wrap">
                    <table className="rca_assist-conn-table">
                      <thead>
                        <tr>
                          <th>Connection / ID</th>
                          <th>Environment</th>
                        ...
                        {connections.map(conn => {
                          ...
                          return (
                            <tr key={conn.connection_id}>
                              <td>
                                <div className="rca_assist-conn-name-col">
                                  <span className="rca_assist-conn-name-text">{conn.connection_name}</span>
                                  <span className="rca_assist-conn-id-sub">{conn.connection_id}</span>
                                </div>
                              </td>
                              <td>
                                <span className="rca_assist-env-badge" style={{ fontSize: 11 }}>
                                  {conn.environment_name}
                                </span>
                              </td>
                              <td>
                                <span className={`rca_assist-routing-tag ${conn.routing_mode || 'direct'}`}>
                              ...
                              <td>
                                {conn.test_status === 'passed' ? (
                                  <span className="rca_assist-conn-status passed" title={conn.last_tested_at ? `Tested at ${new Date(conn.last_tested_at * 1000).toLocaleTimeString()}` : 'Passed'}>
                                    <CheckCircle2 size={12} /> Passed
                                  </span>
                                ) : conn.test_status === 'failed' ? (
                                  <span className="rca_assist-conn-status failed">
                                    <AlertTriangle size={12} /> Failed
                                  </span>
                                ) : (
                                  <span className="rca_assist-conn-status not-run">
                                    <Clock size={12} /> Not Tested
                                  </span>
                                )}
                              </td>
                              <td>
                                <span className={`rca_assist-status-pill ${conn.enabled ? 'active' : 'draft'}`}>
                                  <span className="dot" />
                                  {conn.enabled ? 'Enabled' : 'Disabled'}
                                </span>
                              </td>
                              <td>
                                <div className="rca_assist-conn-actions">
                                  <button
                                    type="button"
                                    className="rca_assist-conn-btn test"
                                  ...
                                  <button
                                    type="button"
                                    className={`rca_assist-conn-btn ${conn.enabled ? 'disable' : 'enable'}`}
          ...
          {(
            <section className="rca_assist-card" id="section-scope">
              <div className="rca_assist-card-header">
                <div className="rca_assist-card-title-wrap">
                  <div className="rca_assist-card-icon"><Layers size={16} /></div>
                  <div>
                    <h2 className="rca_assist-card-title">Scope & Filters</h2>
                    <p className="rca_assist-card-desc">Domain-specific scope and environment mapping configurations.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rca_assist-card-body">
                ...
                {connectorType === 'itsm' && (
                  <div className="rca_assist-domain-scope-box">
                    <div className="rca_assist-form-grid">
                      <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('external_resource') === 'platform_only'}>
                        <div className="rca_assist-field-header">
                          <label className="rca_assist-field-label">Authorized Jira Project Key <span className="req">*</span></label>
                      ...
                      <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('attachment_processing') === 'platform_only'}>
                        <div className="rca_assist-field-header">
                          <label className="rca_assist-field-label">Attachment Processing</label>
                      ...
                      <div className="rca_assist-field-group full-width">
                        <label className="rca_assist-field-label">Default Issue Types</label>
                        <div className="rca_assist-chip-wrap">
                ...
                {connectorType === 'itsm' && <div className="rca_assist-field-group full-width" style={{ marginTop: 20 }} hidden={!isPlatformAdmin && getFieldTier('custom_jql') === 'platform_only'}>
                  <label htmlFor="connector-custom-jql" className="rca_assist-field-label">Custom JQL filter</label>
                ...
                {connectorType === 'log_search' && (
                  <div className="rca_assist-domain-scope-box">
                    <div className="rca_assist-form-grid">
                      <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('external_resource') === 'platform_only'}>
                        <div className="rca_assist-field-header">
                          <label className="rca_assist-field-label">Authorized Splunk Index <span className="req">*</span></label>
                        ...
                        />
                        <span className="rca_assist-field-hint">The current native client strictly accesses one authorized index.</span>
                      </div>

                      <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('max_window_seconds') === 'platform_only'}>
                        <div className="rca_assist-field-header">
                          <label className="rca_assist-field-label">Max Search Window (seconds)</label>
                        ...
                        />
                        <span className="rca_assist-field-hint">Bounded time window capped at 86,400s (24 hours).</span>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {connectorType === 'confluence' && (
                  <div className="rca_assist-form-grid">
                    <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('external_resource') === 'platform_only'}>
                      <label className="rca_assist-field-label">Authorized Space Key</label>
                      <input
                        type="text"
                        className="rca_assist-input"
                ...
                {['qtest', 'signalfx', 'gitlab'].includes(connectorType) && <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('external_resource') === 'platform_only'}>
                  <label htmlFor="connector-resource-scope" className="rca_assist-field-label">{connectorType === 'signalfx' ? 'Detector ID' : 'Project ID'}</label>
                  <input id="connector-resource-scope" className="rca_assist-input" disabled={readOnly || isFieldLocked('external_resource')} value={resourceScope} onChange={event => setResourceScope(event.target.value)}  />
                  <span className="rca_assist-field-hint">The exact authorized resource used for evidence retrieval. Use environment assignments below for different resources per environment.</span>
                </div>}
                {/* Domain Specific: Unix */}
                {connectorType === 'unix' && (
                  <div className="rca_assist-form-grid">
                    <div className="rca_assist-field-group full-width" hidden={!isPlatformAdmin && getFieldTier('external_resource') === 'platform_only'}>
                      <label className="rca_assist-field-label">Authorized Log File Path</label>
                      <input
                        type="text"
                        className="rca_assist-input mono"
                    ...
                    <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('max_tail_bytes') === 'platform_only'}>
                      <label className="rca_assist-field-label">Max Tail Bytes</label>
                      <input
                        type="number"
                        className="rca_assist-input"
                ...
                {connectorType === 'kubernetes' && (
                  <div className="rca_assist-form-grid">
                    <div className="rca_assist-field-group full-width" hidden={!isPlatformAdmin && getFieldTier('external_resource') === 'platform_only'}>
                      <label className="rca_assist-field-label">Authorized Namespace</label>
                      <input
                        type="text"
                        className="rca_assist-input"
                ...
                {environmentDependency === 'dependent' && (isPlatformAdmin || getFieldTier('bindings') !== 'platform_only') && (
                  <div className="rca_assist-env-mappings-wrap" style={{ marginTop: '18px' }}>
                    <div className="rca_assist-section-title-row">
                      <h4 className="rca_assist-subheading">Environment Mappings &amp; Approved Connections</h4>
                        ...
                        {environmentMappings.map((row, idx) => {
                          ...
                          return (
                            <tr key={idx}>
                              <td>
                                <select
                                  className="rca_assist-select-compact"
                                ...
                                {linkedConn && (
                                  <span style={{ display: 'block', marginTop: 2 }}>
                                    {linkedConn.test_status === 'passed' ? (
                                      <span className="rca_assist-conn-status passed" style={{ fontSize: 9.5, padding: '1px 5px' }}>
                                        <CheckCircle2 size={10} /> Passed
                                      </span>
                                    ) : linkedConn.test_status === 'failed' ? (
                                      <span className="rca_assist-conn-status failed" style={{ fontSize: 9.5, padding: '1px 5px' }}>
                                        <AlertTriangle size={10} /> Failed
                                      </span>
                                    ) : (
                                      <span className="rca_assist-conn-status not-run" style={{ fontSize: 9.5, padding: '1px 5px' }}>
                              ...
                              <td>
                                <input
                                  type="text"
                                  className="rca_assist-input-compact mono"
                              ...
                              <td>
                                <input
                                  type="text"
                                  className="rca_assist-input-compact"
        ...
        <div className="rca_assist-grid-col">
          ...
          {templateDefaults && <section className="rca_assist-card connector-shared-defaults">{templateDefaults}</section>}
          {/* Card 4: Operations, Limits & Governance Policy */}
          {(
            <section className="rca_assist-card" id="section-operations">
              <div className="rca_assist-card-header">
                <div className="rca_assist-card-title-wrap">
                  <div className="rca_assist-card-icon"><Activity size={16} /></div>
                  <div>
                    <h2 className="rca_assist-card-title">Advanced settings</h2>
                    <p className="rca_assist-card-desc">Bounded timeouts, rates, and template governance policies.</p>
                  </div>
                </div>
              </div>

              <div className="rca_assist-card-body">
                <div className="rca_assist-form-grid" style={{ marginTop: '16px' }}>
                  {/* Request Timeout (1-120s per candidate_testing.py:305) */}
                  <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('timeout_seconds') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label htmlFor="timeout-input" className="rca_assist-field-label">
                    ...
                    <input
                      ...
                      required
                    />
                    <span className="rca_assist-field-hint">Request timeout is validated against the published connector limits.</span>
                  </div>

                  {/* Maximum Results */}
                  <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('max_results') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label htmlFor="max-results-input" className="rca_assist-field-label">Max Results</label>
                    ...
                    />
                    <span className="rca_assist-field-hint">Result count bound capped per provider limit.</span>
                  </div>

                  {/* Rate Limit */}
                  <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('rate_limit') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label className="rca_assist-field-label">Rate Limit</label>
                  ...
                  {/* Retry Attempts */}
                  <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('retry_attempts') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label className="rca_assist-field-label">Retry Attempts</label>
                  ...
                  {/* Retry Backoff */}
                  <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('retry_backoff') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label className="rca_assist-field-label">Retry Backoff</label>
                    ...
                    <input
                      ...
                      disabled
                    />
                    <span className="rca_assist-field-hint">Wait interval applied between failed retry attempts.</span>
                  </div>

                  {/* Transport Protocol */}
                  <div className="rca_assist-field-group" hidden={!isPlatformAdmin && getFieldTier('protocol') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label className="rca_assist-field-label">Transport</label>
                    ...
                    <input
                      ...
                      disabled
                    />
                    <span className="rca_assist-field-hint">Communication transport protocol implemented by this adapter.</span>
                  </div>

                  {/* Presentation URL */}
                  <div className="rca_assist-field-group full-width" hidden={!isPlatformAdmin && getFieldTier('ui_base_url') === 'platform_only'}>
                    <div className="rca_assist-field-header">
                      <label className="rca_assist-field-label">Presentation URL</label>
                      {renderGovernanceBadge('ui_base_url')}
                    </div>
                    <input
                      type="text"
                      className="rca_assist-input locked mono"
                      value={template?.default_ui_base_url || 'Not declared'}
                      readOnly
                      disabled
                    />
                    <span className="rca_assist-field-hint">Base URL used to construct deep-links into the provider's web user interface.</span>
                  </div>
                </div>

                {/* Repository Release Policy Banner */}
                <div className="rca_assist-policy-safeguard-banner" role="alert" style={{ marginTop: '16px' }}>
          ...
          {(
            <section className="rca_assist-card" id="section-testing">
              <div className="rca_assist-card-header">
                <div className="rca_assist-card-title-wrap">
                  <div className="rca_assist-card-icon"><ShieldCheck size={16} /></div>
                  <div>
                    <h2 className="rca_assist-card-title">Test & enable</h2>
                    <p className="rca_assist-card-desc">15-minute isolated candidate activation gate and reachability tests.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rca_assist-card-body">
                ...
                <div className="rca_assist-gate-banner">
                  <Clock size={16} />
                  <div>
                ...
                <div className="rca_assist-meta-status-row">
                  <div className="rca_assist-candidate-hash-pill">
                    <span>Candidate Hash:</span>
                    <code>{candidateHash ? candidateHash.slice(0, 16) + '...' : 'computing...'}</code>
                  </div>
                  {isDirty && (
                    <span className="rca_assist-dirty-pill">
                ...
                {visibleTestResult && (
                  <div className="rca_assist-stage-results-panel">
                    <h4 className="rca_assist-subheading">Validation Stage Results</h4>
                    <div className="rca_assist-stages-grid">
                      {Object.entries(visibleTestResult.stage_results || {}).map(([stage, res]) => (
                        <div key={stage} className={`rca_assist-stage-pill ${res.status.toLowerCase()}`}>
                ...
                {/* Test Action */}
                <div className="rca_assist-test-actions-box">
                  <button
                    type="button"
                    className="btn btn-primary"
                    onClick={handleTestConnection}
      ...
      {/* ====================================================================
          ...
          ==================================================================== */}
      <footer className="rca_assist-sticky-footer">
        <div className="rca_assist-footer-left">
          {templateDirty && <span className="rca_assist-field-hint">Save or discard shared template changes first.</span>}
          <button
            type="button"
            className="btn btn-secondary"
        ...
        <div className="rca_assist-footer-right">
          {onCancel && (
            <button
              type="button"
              className="btn btn-secondary"
