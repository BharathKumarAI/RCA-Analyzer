...
export const Playground: React.FC<PlaygroundProps> = ({ harness, capability, onTrace }) => {
  ...
  const run = async (value?: string) => {
    ...
    setInput(''); setError(null); setRunning(true); setEvents([]); setRunId(null);
    ...
    try {
      ...
      if (receivedRunId) {
        try { const trace = await fetchStudioTrace(receivedRunId); setEvents(trace.events || []); onTrace?.(trace); } catch { /* stream can finish before trace persistence */ }
      }
    } catch (cause) {
